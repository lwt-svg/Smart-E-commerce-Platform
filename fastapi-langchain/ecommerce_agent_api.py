...
def check_user_cart_and_orders(user_email: str) -> str:
    ...
    try:
        ...
        if not user:
            ...
            if not user:
                return f"未找到Email为 '{user_email}' 的用户。请确认Email地址是否正确。"
...
2. 查询商品的价格、库存、评论等信息
3. 根据用户的预算推荐合适的商品
4. 根据用户的Email查询购物车和订单信息
5. 处理订单相关操作：结算购物车、取消订单、支付订单、查询订单详情
6. 回答用户关于订单和购物的问题
