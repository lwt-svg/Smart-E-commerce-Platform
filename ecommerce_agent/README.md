...
- ✅ **购物车操作** - 添加/查看/删除
- ✅ **订单查询** - 状态跟踪
- ✅ **购买建议** - 基于评论分析生成明确购买建议（推荐/可以考虑/不推荐）
- ✅ **用户画像** - 自动提取用户偏好，个性化推荐

### 🧠 用户画像系统
- ✅ **偏好提取** - 会话结束时自动提取品牌、品类、预算等偏好
- ✅ **向量存储** - 使用 ChromaDB 存储用户画像，支持语义检索
- ✅ **画像注入** - 对话时自动加载用户画像，注入到系统提示词
...
CHROMA_COLLECTION_NAME = "user_preferences"  # 画像集合名称
PROFILE_SUMMARY_MAX_LENGTH = 200        # 画像摘要最大长度
PROFILE_MAX_PREFERENCES = 50            # 最大偏好数量
PROFILE_DECAY_FACTOR = 0.9              # 偏好衰减因子
...
| `/chat/sessions/save` | POST | 保存会话（触发画像更新） |
...
# 保存会话（会自动更新用户画像）
curl -X POST http://localhost:8001/chat/sessions/save \
  -H "Content-Type: application/json" \
  -d '{
    "messages": [...],
...
# 会话结束，自动提取偏好：品牌=华为, 品类=手机, 预算=5000

# 第二次对话（自动加载画像）
用户: 推荐一款平板
助手: [优先推荐华为平板，价格在5000左右...]
```
